...
import { selectNonThinkingSmokeModel, type SmokeStatusModel } from "../src/smoke-model-selection.ts";

const baseUrl = normalizeBaseUrl(process.env.OMLX_BASE_URL ?? "http://127.0.0.1:8008/v1");
...
async function main(): Promise<void> {
	...
	const reasoningModel = process.env.OMLX_REASONING_MODEL ?? models.find((m) => m.thinking_default === true)?.id;
	const nonThinkingModel = process.env.OMLX_NON_THINKING_MODEL ?? selectNonThinkingSmokeModel(models);
	...
	if (!reasoningModel) throw new Error("No reasoning model found. Set OMLX_REASONING_MODEL to run the reasoning smoke.");
	if (!nonThinkingModel) throw new Error("No non-thinking model found. Set OMLX_NON_THINKING_MODEL to run the non-thinking smoke.");
...
async function smokeThinkingBudgetZeroOnly(models: SmokeStatusModel[]): Promise<Record<string, unknown>> {
	...
	if (!forcedModel) {
		...
		return { kind: "thinking_budget_zero_only", status: "skipped", reason: "no_forced_template_model" };
	}
	...
	const thinkingTokens =
		typeof usage?.thinking_tokens === "number" ? usage.thinking_tokens :
		typeof usage?.reasoning_tokens === "number" ? usage.reasoning_tokens : 0;

	if (typeof content !== "string" || content.trim().length === 0) {
		throw new Error(`thinking_budget:0 smoke for ${forcedModel.id} returned no visible content`);
	}
	...
	console.log(`thinking_budget_zero_only ${forcedModel.id} ok thinking_tokens=${thinkingTokens}`);
	return {
...
async function fetchStatusModels(): Promise<SmokeStatusModel[]> {
	...
	if (!response.ok) throw new Error(`/models/status failed: ${response.status} ${await response.text()}`);
	const json = await response.json() as { models?: SmokeStatusModel[] };
	if (!Array.isArray(json.models)) throw new Error("/models/status response missing models array");
	return json.models.filter((model) => typeof model.id === "string");
}

async function smokeChat(model: string, thinking: boolean): Promise<Record<string, unknown>> {
	const request = {
		model,
		messages: [
			{ role: "system", content: "Reply with exactly OK." },
	...
	if (finishReason === "length") {
		throw new Error(`Chat smoke for ${model} was truncated with finish_reason=length`);
	}
...
async function smokeToolCall(model: string | undefined): Promise<Record<string, unknown>> {
	if (!model) throw new Error("No model available for tool-call smoke");
	const request = {
		model,
		messages: [
			{ role: "system", content: "Use tools when the user asks you to call one." },
	...
	if (finishReason === "length") {
		throw new Error(`Tool-call smoke for ${model} was truncated with finish_reason=length`);
	}
	...
	return {
		...
		contentPreview: typeof message?.content === "string" ? message.content.trim().slice(0, 120) : undefined,
		finishReason,
	};
}

async function smokeToolCallStreaming(model: string | undefined): Promise<Record<string, unknown>> {
	if (!model) throw new Error("No model available for streaming tool-call smoke");
	...
	if (!response.ok) throw new Error(`/chat/completions stream failed: ${response.status} ${await response.text()}`);
	if (!response.body) throw new Error("Streaming tool-call smoke returned no response body");
	...
	while (true) {
		...
		for (const line of lines) {
			...
			if (data.includes("\"tool_calls\"") || data.includes("\"tool_call_delta\"")) toolCallDeltaCount += 1;
			try {
				const json = JSON.parse(data);
				const finishReason = json.choices?.[0]?.finish_reason;
				if (typeof finishReason === "string") finishReasonObserved = finishReason;
			} catch {
	...
	if (finishReasonObserved === "length") {
		throw new Error(`Streaming tool-call smoke for ${model} was truncated with finish_reason=length`);
	}
	if (toolCallDeltaCount === 0) {
		throw new Error(`Streaming tool-call smoke for ${model} saw no tool call chunks`);
	}
	console.log(`tool-flow streaming ${model} ok chunks=${sseChunkCount} toolChunks=${toolCallDeltaCount}`);
	return {
...
function buildToolCallRequest(model: string, stream: boolean): Record<string, unknown> {
	return {
		model,
		messages: [
			{ role: "system", content: "Use tools when the user asks you to call one." },
...
async function postChat(payload: Record<string, unknown>): Promise<any> {
	const response = await fetch(`${baseUrl}/chat/completions`, {
		method: "POST",
		headers,
		body: JSON.stringify(payload),
	});
	if (!response.ok) throw new Error(`/chat/completions failed: ${response.status} ${await response.text()}`);
